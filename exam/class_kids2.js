...
function Class (kids) {
  ...
  this.whoIsTheOldest = function () {
    var richestkid = this.kids[0]
    for (var i= 0; i< this.kids.length; i++) {
      if (this.kids[i].money > richestkid.money) {
        richestkid = this.kids[i];
      }
    }
    var poorestkid = this.kids[0];
    for (var i = 0; i< this.kids.length; i++) {
      if (this.kids[i].money < poorestkid.money) {
        poorestkid = this.kids[i];
      }
    }
    if (richestkid.age > poorestkid.age) {
      return richestkid.name;
    }
    else {
      return poorestkid.name;
    }
  }
